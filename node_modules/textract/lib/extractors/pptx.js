var xpath = require( 'xpath' )
  ...
  , yauzl = require( 'yauzl' )
  , util = require( '../util' )
  , slideMatch = /^ppt\/(slides\/slide|notesSlides\/notesSlide)/
  ;
    ...
    zipfile.on( 'entry', function( entry ) {
      if ( slideMatch.test( entry.fileName ) ) {
        util.getTextFromZipFile( zipfile, entry, function( err2, text ) {
          ...
          var slide;
          if (entry.fileName.includes("slides/slide")) {
            slide = +entry.fileName.replace( 'ppt/slides/slide', '' ).replace( '.xml', '' );
          }
          else if (entry.fileName.includes("notesSlides/notesSlide")) {
            slide = +entry.fileName.replace( 'ppt/notesSlides/notesSlide', '' ).replace( '.xml', '' );
          }
          else {
            throw new Error(`Internal error - unexpected entry.fileName="${entry.filename}".`);
          }
